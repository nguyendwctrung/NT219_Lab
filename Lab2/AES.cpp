#include "AES.h"
#include <stdexcept>
#include <algorithm>
#include <sstream>

AES::AES(const std::vector<uint8_t>& key, uint8_t key_length)
        : key(key),
          key_length(key_length),
          S_BOX({
                        }),
          INV_S_BOX({
                            }),
          RCON({
                       {0x01, 0x00, 0x00, 0x00},
                       {0x02, 0x00, 0x00, 0x00},
                       {0x04, 0x00, 0x00, 0x00},
                       {0x08, 0x00, 0x00, 0x00},
                       {0x10, 0x00, 0x00, 0x00},
                       {0x20, 0x00, 0x00, 0x00},
                       {0x40, 0x00, 0x00, 0x00},
                       {0x80, 0x00, 0x00, 0x00},
                       {0x1B, 0x00, 0x00, 0x00},
                       {0x36, 0x00, 0x00, 0x00}})
{
    if (key.size() != key_length || (key_length != 16 && key_length != 24 && key_length != 32)) {
        std::stringstream ss;
        ss << "Invalid key length: " << static_cast<int>(key_length) << " bytes. Must be 16, 24, or 32 bytes, and match key size (" << key.size() << " bytes).";
        throw std::invalid_argument(ss.str());
    }
    round_keys = key_expansion(key, key_length);
}

std::vector<std::vector<uint8_t>> AES::key_expansion(std::vector<uint8_t> key, uint8_t length) {
    if (key.size() != length) {
        std::stringstream ss;
        ss << "Key size (" << key.size() << " bytes) does not match specified length (" << static_cast<int>(length) << " bytes).";
        throw std::invalid_argument(ss.str());
    }
    this->key = key;
    this->key_length = length;

    if (length == 16) {
        return key_expansion_128();
    } else if (length == 24) {
        return key_expansion_192();
    } else if (length == 32) {
        return key_expansion_256();
    } else {
        std::stringstream ss;
        ss << "Invalid key length: " << static_cast<int>(length) << " bytes. Supported lengths are 16, 24, or 32 bytes.";
        throw std::invalid_argument(ss.str());
    }
}

std::vector<uint8_t> AES::sub_word(const std::vector<uint8_t>& word) {
    std::vector<uint8_t> result(word.size());
    std::transform(word.begin(), word.end(), result.begin(),
                   [this](uint8_t b) { return S_BOX[b]; });
    return result;
}

std::vector<uint8_t> AES::rot_word(std::vector<uint8_t> word) {
    if (word.size() != 4) {
        throw std::invalid_argument("rot_word expects a 4-byte word");
    }
    std::rotate(word.begin(), word.begin() + 1, word.end());
    return word;
}

std::vector<std::vector<uint8_t>> AES::key_expansion_128() {
    if (key.size() != 16) {
        throw std::invalid_argument("Key size must be 16 bytes for AES-128");
    }
    constexpr uint8_t key_words = 4;
    constexpr uint8_t total_words = 44;
    std::vector<std::vector<uint8_t>> round_keys(total_words, std::vector<uint8_t>(4));

    for (uint8_t i = 0; i < key_words; ++i) {
        round_keys[i].assign(key.begin() + i * 4, key.begin() + (i + 1) * 4);
    }

    for (uint8_t i = key_words; i < total_words; ++i) {
        std::vector<uint8_t> temp = round_keys[i - 1];
        if (i % key_words == 0) {
            temp = sub_word(rot_word(temp));
            for (uint8_t j = 0; j < 4; ++j) {
                temp[j] ^= RCON[(i - key_words) / key_words][j];
            }
        }
        for (uint8_t j = 0; j < 4; ++j) {
            round_keys[i][j] = round_keys[i - key_words][j] ^ temp[j];
        }
    }
    return round_keys;
}

std::vector<std::vector<uint8_t>> AES::key_expansion_192() {
    if (key.size() != 24) {
        throw std::invalid_argument("Key size must be 24 bytes for AES-192");
    }
    constexpr uint8_t key_words = 6;
    constexpr uint8_t total_words = 52;
    std::vector<std::vector<uint8_t>> round_keys(total_words, std::vector<uint8_t>(4));

    for (uint8_t i = 0; i < key_words; ++i) {
        round_keys[i].assign(key.begin() + i * 4, key.begin() + (i + 1) * 4);
    }

    for (uint8_t i = key_words; i < total_words; ++i) {
        std::vector<uint8_t> temp = round_keys[i - 1];
        if (i % key_words == 0) {
            temp = sub_word(rot_word(temp));
            for (uint8_t j = 0; j < 4; ++j) {
                temp[j] ^= RCON[(i - key_words) / key_words][j];
            }
        }
        for (uint8_t j = 0; j < 4; ++j) {
            round_keys[i][j] = round_keys[i - key_words][j] ^ temp[j];
        }
    }
    return round_keys;
}

std::vector<std::vector<uint8_t>> AES::key_expansion_256() {
    if (key.size() != 32) {
        throw std::invalid_argument("Key size must be 32 bytes for AES-256");
    }
    constexpr uint8_t key_words = 8;
    constexpr uint8_t total_words = 60;
    std::vector<std::vector<uint8_t>> round_keys(total_words, std::vector<uint8_t>(4));

    for (uint8_t i = 0; i < key_words; ++i) {
        round_keys[i].assign(key.begin() + i * 4, key.begin() + (i + 1) * 4);
    }

    for (uint8_t i = key_words; i < total_words; ++i) {
        std::vector<uint8_t> temp = round_keys[i - 1];
        if (i % key_words == 0) {
            temp = sub_word(rot_word(temp));
            for (uint8_t j = 0; j < 4; ++j) {
                temp[j] ^= RCON[(i - key_words) / key_words][j];
            }
        } else if (i % key_words == 4) {
            temp = sub_word(temp);
        }
        for (uint8_t j = 0; j < 4; ++j) {
            round_keys[i][j] = round_keys[i - key_words][j] ^ temp[j];
        }
    }
    return round_keys;
}

std::vector<std::vector<uint8_t>> AES::sub_bytes(std::vector<std::vector<uint8_t>> state) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            state[i][j] = S_BOX[state[i][j]];
        }
    }
    return state;
}

std::vector<std::vector<uint8_t>> AES::shift_rows(std::vector<std::vector<uint8_t>> state) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    std::rotate(state[1].begin(), state[1].begin() + 1, state[1].end());
    std::rotate(state[2].begin(), state[2].begin() + 2, state[2].end());
    std::rotate(state[3].begin(), state[3].begin() + 3, state[3].end());
    return state;
}

std::vector<std::vector<uint8_t>> AES::mix_columns(std::vector<std::vector<uint8_t>> state) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    auto gmul = [](uint8_t a, uint8_t b) -> uint8_t {
        uint8_t p = 0;
        for (uint8_t i = 0; i < 8; ++i) {
            if (b & 1) p ^= a;
            bool hi_bit_set = a & 0x80;
            a <<= 1;
            if (hi_bit_set) a ^= 0x1b;
            b >>= 1;
        }
        return p;
    };

    for (uint8_t i = 0; i < 4; ++i) {
        std::vector<uint8_t> col(4);
        for (uint8_t j = 0; j < 4; ++j) {
            col[j] = state[j][i];
        }
        state[0][i] = gmul(0x02, col[0]) ^ gmul(0x03, col[1]) ^ col[2] ^ col[3];
        state[1][i] = col[0] ^ gmul(0x02, col[1]) ^ gmul(0x03, col[2]) ^ col[3];
        state[2][i] = col[0] ^ col[1] ^ gmul(0x02, col[2]) ^ gmul(0x03, col[3]);
        state[3][i] = gmul(0x03, col[0]) ^ col[1] ^ col[2] ^ gmul(0x02, col[3]);
    }
    return state;
}

std::vector<std::vector<uint8_t>> AES::inv_sub_bytes(std::vector<std::vector<uint8_t>> state) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            state[i][j] = INV_S_BOX[state[i][j]];
        }
    }
    return state;
}

std::vector<std::vector<uint8_t>> AES::inv_shift_rows(std::vector<std::vector<uint8_t>> state) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    std::rotate(state[1].rbegin(), state[1].rbegin() + 1, state[1].rend());
    std::rotate(state[2].rbegin(), state[2].rbegin() + 2, state[2].rend());
    std::rotate(state[3].rbegin(), state[3].rbegin() + 3, state[3].rend());
    return state;
}

std::vector<std::vector<uint8_t>> AES::inv_mix_columns(std::vector<std::vector<uint8_t>> state) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    auto gmul = [](uint8_t a, uint8_t b) -> uint8_t {
        uint8_t p = 0;
        for (uint8_t i = 0; i < 8; ++i) {
            if (b & 1) p ^= a;
            bool hi_bit_set = a & 0x80;
            a <<= 1;
            if (hi_bit_set) a ^= 0x1b;
            b >>= 1;
        }
        return p;
    };

    for (uint8_t i = 0; i < 4; ++i) {
        std::vector<uint8_t> col(4);
        for (uint8_t j = 0; j < 4; ++j) {
            col[j] = state[j][i];
        }
        state[0][i] = gmul(0x0E, col[0]) ^ gmul(0x0B, col[1]) ^ gmul(0x0D, col[2]) ^ gmul(0x09, col[3]);
        state[1][i] = gmul(0x09, col[0]) ^ gmul(0x0E, col[1]) ^ gmul(0x0B, col[2]) ^ gmul(0x0D, col[3]);
        state[2][i] = gmul(0x0D, col[0]) ^ gmul(0x09, col[1]) ^ gmul(0x0E, col[2]) ^ gmul(0x0B, col[3]);
        state[3][i] = gmul(0x0B, col[0]) ^ gmul(0x0D, col[1]) ^ gmul(0x09, col[2]) ^ gmul(0x0E, col[3]);
    }
    return state;
}

std::vector<std::vector<uint8_t>> AES::add_round_key(std::vector<std::vector<uint8_t>> state, uint8_t round_number) {
    if (state.size() != 4 || state[0].size() != 4) {
        throw std::invalid_argument("State must be a 4x4 matrix");
    }
    if (round_number * 4 >= round_keys.size()) {
        throw std::invalid_argument("Invalid round number");
    }
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            state[j][i] ^= round_keys[round_number * 4 + i][j];
        }
    }
    return state;
}

std::vector<uint8_t> AES::encrypt(std::vector<uint8_t> data) {
    if (data.size() != 16) {
        std::stringstream ss;
        ss << "Input data size (" << data.size() << " bytes) must be 16 bytes for AES encryption";
        throw std::invalid_argument(ss.str());
    }
    if (round_keys.empty()) {
        round_keys = key_expansion(key, key_length);
    }

    std::vector<std::vector<uint8_t>> state(4, std::vector<uint8_t>(4));
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            state[j][i] = data[i * 4 + j];
        }
    }

    uint8_t num_rounds;
    switch (key_length) {
        case 16: num_rounds = 10; break;
        case 24: num_rounds = 12; break;
        case 32: num_rounds = 14; break;
        default:
            std::stringstream ss;
            ss << "Invalid key length: " << static_cast<int>(key_length) << " bytes";
            throw std::invalid_argument(ss.str());
    }

    state = add_round_key(state, 0);
    for (uint8_t round = 1; round < num_rounds; ++round) {
        state = sub_bytes(state);
        state = shift_rows(state);
        state = mix_columns(state);
        state = add_round_key(state, round);
    }
    state = sub_bytes(state);
    state = shift_rows(state);
    state = add_round_key(state, num_rounds);

    std::vector<uint8_t> encrypted_data(16);
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            encrypted_data[i * 4 + j] = state[j][i];
        }
    }
    return encrypted_data;
}

std::vector<uint8_t> AES::decrypt(std::vector<uint8_t> ciphertext) {
    if (ciphertext.size() != 16) {
        std::stringstream ss;
        ss << "Ciphertext size (" << ciphertext.size() << " bytes) must be 16 bytes for AES decryption";
        throw std::invalid_argument(ss.str());
    }
    if (round_keys.empty()) {
        round_keys = key_expansion(key, key_length);
    }

    std::vector<std::vector<uint8_t>> state(4, std::vector<uint8_t>(4));
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            state[j][i] = ciphertext[i * 4 + j];
        }
    }

    uint8_t num_rounds;
    switch (key_length) {
        case 16: num_rounds = 10; break;
        case 24: num_rounds = 12; break;
        case 32: num_rounds = 14; break;
        default:
            std::stringstream ss;
            ss << "Invalid key length: " << static_cast<int>(key_length) << " bytes";
            throw std::invalid_argument(ss.str());
    }

    state = add_round_key(state, num_rounds);
    for (uint8_t round = num_rounds - 1; round > 0; --round) {
        state = inv_shift_rows(state);
        state = inv_sub_bytes(state);
        state = add_round_key(state, round);
        state = inv_mix_columns(state);
    }
    state = inv_shift_rows(state);
    state = inv_sub_bytes(state);
    state = add_round_key(state, 0);

    std::vector<uint8_t> decrypted_data(16);
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            decrypted_data[i * 4 + j] = state[j][i];
        }
    }
    return decrypted_data;
}